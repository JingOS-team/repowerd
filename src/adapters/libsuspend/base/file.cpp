...
TemporaryDir::~TemporaryDir() {
  ...
  auto callback = [](const char* child, const struct stat*, int file_type, struct FTW*) -> int {
    switch (file_type) {
      ...
      case FTW_DNR:
        if (rmdir(child) == -1) {
         /*[dba] PLOG(ERROR)*/  //![dba]  qDebug() << "rmdir " << child;
        }
        break;
      case FTW_NS:
      default:
      ...
      case FTW_SLN:
        if (unlink(child) == -1) {
         /*[dba] PLOG(ERROR)*/  //![dba]  qDebug() << "unlink " << child;
        }
        break;
    }
    return 0;
...
bool WriteStringToFile(const std::string& content, const std::string& path,
  ...
  if (fd == -1) {
   /*[dba] PLOG(ERROR)*/  //![dba]  qDebug() << "android::WriteStringToFile open failed";
    return false;
  }
  ...
  if (fchmod(fd, mode) == -1) {
   /*[dba] PLOG(ERROR)*/  //![dba]  qDebug() << "android::WriteStringToFile fchmod failed";
    return CleanUpAfterFailedWrite(path);
  }
  if (fchown(fd, owner, group) == -1) {
   /*[dba] PLOG(ERROR)*/  //![dba]  qDebug() << "android::WriteStringToFile fchown failed";
    return CleanUpAfterFailedWrite(path);
  }
  if (!WriteStringToFd(content, fd)) {
   /*[dba] PLOG(ERROR)*/  //![dba]  qDebug() << "android::WriteStringToFile write failed";
    return CleanUpAfterFailedWrite(path);
  }
  return true;
}
