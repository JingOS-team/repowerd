...
double constexpr critical_percentage{0.0};
double constexpr non_critical_percentage{1.0};
...
{
    bool const is_display_device =  true /*device_path == display_device_path*/;

    if (!is_display_device && batteries.find(device_path) == batteries.end())
        return;

    log->log(log_tag, "\e[0;36m[dba_DEBUG] %s:%d %s: device_path: %s \e[0m\n",__FILE__,__LINE__,__FUNCTION__, device_path.c_str() );
    ...
    log->log(log_tag, "\e[0;36m[dba_DEBUG] %s:%d %s: new_info.state: %d old_info.state:%d \e[0m\n", __FILE__,__LINE__,__FUNCTION__, new_info.state, old_info.state);
    ...
    log->log(log_tag, "\e[0;36m[dba_DEBUG] %s:%d %s: critical: %d change:%d \e[0m\n",__FILE__,__LINE__,__FUNCTION__, critical, change);
    if (critical)
    {
        highest_seen_percentage = 0.0;
