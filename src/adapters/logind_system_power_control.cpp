...
static void wakeup_callback(bool success){
    ...
    if(success){
        ...
        acquire_wake_lock("repowerd_wakeup_callback 2000000000");
    }
}
/****************************************************************************/
...
void repowerd::LogindSystemPowerControl::allow_automatic_suspend(std::string const& id )
{
    log->log(log_tag, "LogindSystemPowerControl::allow_automatic_suspend %s", id.c_str());
    if(!id.empty()){
        std::lock_guard<std::mutex> lock{suspend_mutex};
        release_wake_lock(id.c_str());
        log->log(log_tag, "allow_suspend(%s)", id.c_str());
    }
}

void repowerd::LogindSystemPowerControl::disallow_automatic_suspend(std::string const& id)
{
    log->log(log_tag, "LogindSystemPowerControl::disallow_automatic_suspend %s", id.c_str());
    if(!id.empty()){
        std::lock_guard<std::mutex> lock{suspend_mutex};
        acquire_wake_lock(id.c_str());
