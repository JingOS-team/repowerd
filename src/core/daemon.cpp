    ...
    registrations.push_back(
        client_requests->register_turn_on_display_handler(
            [this] (std::string const& id, pid_t pid)
            {
                enqueue_action_to_sessions(
                    sessions_for_pid(pid),
                    [this, id] (Session* s) { s->state_machine->handle_turn_on_display(); });
            }));
    registrations.push_back(
        client_requests->register_turn_off_display_handler(
            [this] (std::string const& id, pid_t pid)
            {
                enqueue_action_to_sessions(
                    sessions_for_pid(pid),
                    [this, id] (Session* s) { s->state_machine->handle_turn_off_display(); });
            }));
    registrations.push_back(
