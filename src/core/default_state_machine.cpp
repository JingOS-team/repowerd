...
namespace
{
char const* const suspend_id = "repowerd_default_state_machine";
...
void repowerd::DefaultStateMachine::handle_no_active_call()
{
    ...
    if (display_power_mode == DisplayPowerMode::on)
    {
        log->log(log_tag, "display_power_mode == DisplayPowerMode::on");
        brighten_display();
    ...
    else
    {
        ...
        {
            log->log(log_tag, "proximity_sensor->proximity_state() == ProximityState::far");
            turn_on_display_with_reduced_timeout(DisplayPowerChangeReason::call_done);
        }
        else
        {
            log->log(log_tag, "enable_proximity");
...
void repowerd::DefaultStateMachine::handle_notification()
{
    ...
    else
    {
        if (proximity_sensor->proximity_state() == ProximityState::far)
        {
            log->log(log_tag, "proximity_state() == ProximityState::far");
...
void repowerd::DefaultStateMachine::handle_power_button_press()
{
    ...
    {
        log->log(log_tag, "display_power_mode == DisplayPowerMode::off");
...
{
    log->log(log_tag, "handle_power_button_release %d %d",display_power_mode_at_power_button_press, treat_power_button_as_user_activity);
...
void repowerd::DefaultStateMachine::handle_allow_suspend()
{
    ...
    if (display_power_mode == DisplayPowerMode::off &&
            display_power_mode_reason != DisplayPowerChangeReason::proximity)
...
{
    //turn_on_display_without_timeout(DisplayPowerChangeReason::power_button);
    turn_on_display_with_normal_timeout(DisplayPowerChangeReason::power_button);
}
void repowerd::DefaultStateMachine::handle_turn_off_display()
{
    turn_off_display(DisplayPowerChangeReason::power_button);
}
...
{
    log->log(log_tag, "turn_off_display %d:", reason);
    ...
    if (reason != DisplayPowerChangeReason::proximity && suspend_allowed)
        system_power_control->allow_automatic_suspend(suspend_id);
}
