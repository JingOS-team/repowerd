...
//! [dba debug: 2021-08-04]
using TunOnDisplayHandler = std::function<void(std::string const&, pid_t)>;
using TunOffDisplayHandler = std::function<void(std::string const&, pid_t)>;
...
public:
    ...
    virtual HandlerRegistration register_turn_on_display_handler(
        TunOnDisplayHandler const& handler) = 0;
    virtual HandlerRegistration register_turn_off_display_handler(
        TunOffDisplayHandler const& handler) = 0;
protected:
