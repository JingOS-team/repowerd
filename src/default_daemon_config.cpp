...
#include "adapters/jingos_display.h"
#include "adapters/jingos_power_button.h"
#include "adapters/jingos_screen_service.h"
#include "adapters/jingos_user_activity.h"
...
{
    return the_jingos_screen_service();
}
...
{
    return the_jingos_power_button();
}
        ...
        try
        {
            if (!system_power_control)
            {
                system_power_control = std::make_shared<LogindSystemPowerControl>(
                    the_log(),
                    the_dbus_bus_address());
                the_log()->log(log_tag, "make_shared LogindSystemPowerControl");
            }
        }
        catch (std::exception const& e)
        {
        ...
        try
        {
            if (!system_power_control)
            {
                system_power_control = std::make_shared<LibsuspendSystemPowerControl>(
                    the_log());
                the_log()->log(log_tag, "make_shared LibsuspendSystemPowerControl");
            }
        }
        catch (std::exception const& e)
        {
...
{
    if (!user_activity)
        user_activity = std::make_shared<JingosUserActivity>(the_dbus_bus_address());
...
repowerd::DefaultDaemonConfig::the_device_config()
{
    if (!device_config)
    {
        the_log()->log(log_tag,"\e[0;36m[dba_DEBUG] %s:%d %s \e[0m\n",__FILE__,__LINE__,__FUNCTION__);
...
repowerd::DefaultDaemonConfig::the_light_sensor()
{
    ...
    try
    {
        light_sensor = std::make_shared<UbuntuLightSensor>(the_dbus_bus_address(), the_log());
    }
    catch (std::exception const& e)
    {
...
std::shared_ptr<repowerd::JingosDisplay>
repowerd::DefaultDaemonConfig::the_jingos_display()
{
    if (!jingos_display)
    {
        jingos_display = std::make_shared<JingosDisplay>(
            the_log(),
            the_dbus_bus_address());
    }
    return jingos_display;
}

std::shared_ptr<repowerd::JingosScreenService>
repowerd::DefaultDaemonConfig::the_jingos_screen_service()
{
    if (!jingos_screen_service)
    {
        jingos_screen_service = std::make_shared<JingosScreenService>(
            the_wakeup_service(),
    ...
    return jingos_screen_service;
}

std::shared_ptr<repowerd::JingosPowerButton>
repowerd::DefaultDaemonConfig::the_jingos_power_button()
{
    if (!jingos_power_button)
        jingos_power_button = std::make_shared<JingosPowerButton>(the_dbus_bus_address(), the_log());
    return jingos_power_button;
}
